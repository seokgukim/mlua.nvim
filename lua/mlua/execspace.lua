...
---@type table<number, boolean> Track if decorations are enabled for a buffer
					...
					if type(line) == "number" and line >= 0 then
						...
						if line < line_count then
							pcall(vim.api.nvim_buf_set_extmark, bufnr, ns_id, line, 0, {
								virt_text = { { " " .. label_info.text, label_info.hl } },
								virt_text_pos = "eol",
								priority = 100,
							})
						end
...
function M.request_decorations(client, bufnr)
	if not client or not vim.api.nvim_buf_is_valid(bufnr) or not buffer_enabled[bufnr] then
		return
	end
...
function M.setup_for_buffer(client, bufnr)
	...
	-- Handle the server-side decorationChanged notification if translated by proxy
	-- Note: This requires the handler to be registered in lsp.lua
