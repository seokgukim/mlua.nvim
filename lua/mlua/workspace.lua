...
local basic_keywords = {
  ...
  "true", "until", "while", "type", "table", "string", "number", "boolean",
  "math", "io", "os", "coroutine", "debug", "package", "require", "print",
  "pairs", "ipairs", "next", "pcall", "xpcall", "select", "unpack",
  "rawget", "rawset", "setmetatable", "getmetatable", "rawequal", "tonumber",
  "tostring", "error", "assert", "load", "loadfile", "dofile", "collectgarbage",
}
...
local function extract_tokens_from_lines(lines)
  ...
  for _, line in ipairs(lines) do
    ...
    for token in line:gmatch("[%w%d_]+") do
      ...
      if token_normalized == '' then
        goto continue
      end

      if basic_keyword_set[token_normalized] then
        goto continue  -- Skip basic keywords
      end
    ...
    if exact_paths then
      ...
      for i = 1, #path_fuzzy[root_dir] do
        ...
        if path_state[root_dir][path] ~= false then
          goto continue  -- Not in state to load or already loaded
        end
